...
class ORConv2d(Conv2d):
    def __init__(self, in_channels, out_channels, arf_config, kernel_size, stride=1,
        ...
        self.binfunc = BinActivateFunc_bireal.apply
    ...
    def forward(self, input):
        #d = torch.sqrt(Wacc)
        alpha = torch.mean(torch.abs(self.rotate_arf())) 
        weight = self.binfunc(self.rotate_arf())
        input = self.binfunc(input)
        return F.conv2d(input, weight, self.bias, self.stride,
                self.padding, self.dilation, self.groups)*alpha
